...
高级 `Json` 过滤仅在使用不同语法的 `path` 选项下被 [PostgreSQL](../../../database-connectors/postgresql) 和 [MySQL](../../../database-connectors/mysql) 支持。PostgreSQL 不支持[对数组中的对象键值进行过滤](#filtering-on-object-key-value-inside-array) 。
