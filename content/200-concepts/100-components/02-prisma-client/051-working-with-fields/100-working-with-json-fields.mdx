...
高级 `Json` 过滤在 [2.23.0 及更高版本](https://github.com/prisma/prisma/releases/tag/2.23.0) 中作为预览功能提供。在早期版本中，您可以[过滤精确的 `Json` 字段值](#filter-on-exact-field-value)。
...
在 [2.23.0](https://github.com/prisma/prisma/releases/tag/2.23.0) 及更高版本中，您可以过滤 JSON 块内的特定属性。在以下示例中， `extendedPetsData` 是一个一层、未嵌套的 JSON 对象：
...
- 你必须设置 `array_contains` 为 JSON 对象，而不是字符串。如果您使用字符串，Prisma 会转义引号并且查询将不会返回结果。例如：
