...
区分大小写影响数据的 **筛选** 和 **排序**，并由你的 [数据库排序规则](#database-collation-and-case-sensitivity) 决定。根据你的设置，对数据进行排序和筛选会产生不同的结果：
...
如果使用 MongoDB 连接器，[Prisma Client](.) 使用正则表达式规则来启用不区分大小写的筛选。连接器 _不_ 使用 [MongoDB 排序规则](https://docs.mongodb.com/manual/reference/collation/)。
...
## 不区分大小写筛选选项 {#options-for-case-insensitive-filtering}

使用 Prisma Client 支持不区分大小写过滤的推荐方法取决于你的基础提供程序。

### PostgreSQL 提供程序

PostgreSQL 默认使用 [确定性排序](https://www.postgresql.org/docs/current/collation.html#COLLATION-NONDETERMINISTIC) ，这意味着筛选是 **区分大小写** 的。要支持不区分大小写的筛选，请在每个字段的基础上使用 `mode: 'insensitive'` 属性。
...
请参阅：[筛选 (不区分大小写的过滤)](filtering-and-sorting#case-insensitive-filtering)

#### 附加说明

- 不能将不区分大小写的筛选与 C 排序一起使用
- [`citext`](https://www.postgresql.org/docs/12/citext.html) 列始终不区分大小写，并且不受 `mode` 的影响

#### 性能

如果你严重依赖于不区分大小写的筛选，请考虑 [在 PostgreSQL 数据库中创建索引](https://www.postgresql.org/docs/current/indexes.html) 来提高性能：
...
- 为了支持不区分大小写的筛选，你必须使用不区分大小写的（`_ci`）排序规则。Prisma Client 不支持 MySQL 提供程序的 `mode` 筛选器属性。

### MongoDB 提供程序

若要支持不区分大小写的筛选，请在每个字段的基础上使用 `mode: 'insensitive'` 属性：
