...
查询引擎管理数据库连接的　**连接池**。当 Prisma Client 打开到数据库的　_第一个_　连接时会创建池，可以通过以下两种方式之一触发：
...
1. 查询引擎在连接上创建并将其添加到连接池中。
1. 当查询进来时，查询引擎从池中保留一个连接来处理查询。
1. 如果连接池中没有空闲连接可用，查询引擎会打开额外的数据库连接并将它们添加到连接池中，直到数据库连接数达到定义的 `connection_limit` 的限制 。
1. 如果查询引擎无法从池中保留连接，则会将查询添加到内存中的 FIFO（先进先出）队列。FIFO 意味着查询按照它们进入队列的顺序进行处理。
1. 如果查询引擎在 **[默认超时时间限制](#default-pool-timeout) 之前** 无法处理队列中的查询，它会抛出带有该查询 `P2024` 错误代码的异常并移至队列中的下一个。

如果一直遇到池超时错误，则需要 [优化连接池](../../../../guides/performance-and-optimization/connection-management#optimizing-the-connection-pool) <span class="guide"></span>。
...
`num_physical_cpus` 代表机器上物理 CPU 的数量。如果你的机器有 **4** 个物理 CPU，则你的连接池将包含 **9** (`4 * 2 + 1 = 9`)。

虽然公式代表了一个很好的起点，但 [推荐的连接限制](../../../../guides/performance-and-optimization/connection-management#recommended-connection-pool-size) <span class="guide"></span> 还取决于你的部署范例 - 特别是如果你使用无服务器。

#### 设置连接池大小　{#setting-the-connection-pool-size}

You can specify the number of connections by explicitly setting the `connection_limit` parameter in your database connection URL. For example, with the following `datasource` configuration in your [Prisma schema](../../prisma-schema) the 连接池 will have exactly five connections:
...
不能增加 `connection_limit` 超出底层数据库可以支持的范围。这在无服务器环境中是一个特殊的挑战，其中每个函数管理一个 `PrismaClient` 实例 - 以及它自己的连接池。
