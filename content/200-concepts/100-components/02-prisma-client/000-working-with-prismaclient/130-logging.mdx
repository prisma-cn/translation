...
title: '配置日志记录'
metaTitle: '配置日志记录 (概念)'
metaDescription: '了解如何配置 Prisma Client 记录它发送到数据库的原始 SQL 查询和其他信息。'
---
...
下面的例子订阅的所有 `query` 事件和写入 `duration` ，并 `query` 给控制台：
...
准确的 [事件 (`e`) 类型和可用的属性](/reference/api-reference/prisma-client-reference#event-types) <span class="api"></span> 取决于日志级别。
