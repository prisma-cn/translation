...
此 schema 生成的 Prisma Client 代码包含以下 `User` 类型的表示：
...
在某些情况下，你可能需要生成的 `User` 类型的 _变体_。例如，当你有一个函数需要一个带有 `posts` 关系的 `User` 模型实例时。或者，当你需要一个类型在应用程序代码中只传递 `User` 模型的 `email` 和 `name` 字段时。
