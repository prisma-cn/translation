---
title: 'CRUD 增删改查'
metaTitle: 'CRUD 增删改查(参考)'
metaDescription: '如何使用 Prisma Client 执行 CRUD 增删改查。'
tocDepth: 5
---
...
对于 **关系型数据库**，使用 `db push` 命令将示例 schema 添加到你自己的数据库中
...
Prisma Client 在 [2.20.0](https://github.com/prisma/prisma/releases/2.20.0) 和更高版本中支持将大量插入作为 基础可用 功能。
...
> **注意**：`$transaction` 中的多个 `create` 语句会生成多个 `INSERT` 语句。
...
<div class="videoWrapper">
  <iframe
    ...
    border="0"
    framespacing="0"
    src="//player.bilibili.com/player.html?aid=931453747&bvid=BV1UM4y1T7QF&cid=403685728&page=4"
    frameborder="0"
...
你也可以 [对你的结果进行分页](pagination)。

### 获取与特定条件匹配的第一条记录

以下 [`findFirst`](../../../reference/api-reference/prisma-client-reference#findfirst) <span class="api"></span> 查询返回 至少有一个帖子有超过 100 个赞的 _最新创建的用户_：
