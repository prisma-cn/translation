...
你可以将聚合与筛选和排序结合起来。例如，以下查询返回用户的平均年龄：
- 按 `age` 升序排列
...
在**分组之前**，使用 `where` 筛选所有记录。以下示例按国家对用户进行分组，并对个人资料浏览数进行汇总，但仅包括电子邮件地址包含 `prisma.io` 的用户：
...
使用 `having` 按聚合值（如字段的总和或平均值）而不是单个记录筛选 **整个组**，例如，仅返回 _平均_ `人资料浏览数` 大于 100 的组：
...
> **注意**：在 `having` 中，你只能筛选 `by` 中可用的聚合值 _或_ 字段。
