...
- 选填的 **[类型修饰符（type modifiers）](#类型修饰符)**
- 选填的 **[属性（attributes）](#定义属性)**，包含 [数据库原生类型映射](#原生类型映射)

字段类型决定了它的 _结构_，且分为以下两类：

- [标量类型](data-model#标量字段)（Scalar types，包括 [枚举](data-model#定义枚举))，映射至数据库列（关系型数据库）或文件字段（MongoDB）- 例如，[`String`](../../../reference/api-reference/prisma-schema-reference#string) <span class="api"></span> 或 [`Int`](../../../reference/api-reference/prisma-schema-reference#int) <span class="api"></span>
- 模型类型（Model types，该字段因此被称为 [关系字段](relations#关系字段)）- 例如 `Post` 或 `Comment[]`。
...
以下例子用若干标量类型扩展了 `Comment` 和 `Tag` 模型。部分字段含有 [定义属性](#定义属性):
...
除此之外，在[内省 Introspection](../../introspection)期间，_只有_ 在底层原生类型 **不是默认类型** 时，schema 中才会加入类型属性。例如，如果你使用 PostgreSQL 作为 provider，`String` 字段底层对应的 `text` 原生类型没有类型属性（type attribute）。

#### 好处和操作流程（Benefits and workflows）

- 控制 [Prisma Migrate](../prisma-migrate) 在数据库中创建的 **实际原生类型** - 例如，一个 `String` 字段可以是 `@db.VarChar(200)` 或 `@db.Char(50)`
- 内省(introspect) 时可得到 **更精确的 schema**
