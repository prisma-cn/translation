...
<div style={{ textAlign: 'center', margin: '2em auto' }}>
  <iframe
    width="560"
    height="315"
    src="//player.bilibili.com/player.html?aid=931453747&bvid=BV1UM4y1T7QF&cid=403685091&page=2"
    title="whatisprisma"
    frameborder="0"
...
任何使用 Prisma 的项目都从一个 [Prisma schema 文件](../../components/prisma-schema) 开始。这个 Prisma schema 文件允许开发者通过一种直观的数据建模语言来定义自己的 _应用模型_。它还包含了与数据库的连接和一个*生成器（generator）*。
...
**不支持 MongoDB**<br />
内省（Introspection）和 Prisma Migrate 暂未支持 MongoDB。支持 MongoDB 的能力仍处于预览阶段，如果您要使用它，你必须以匹配 MongoDB 中的数据的方式来设计你的 Prisma schema。
...
4. 生成 Prisma Client
5. 在应用代码中使用 Prisma Client 访问数据库
