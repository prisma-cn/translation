...
| `connect_timeout`  | No       | `5`                    | 最大连接建立超时时间，`0` 代表无限制                                                                     |
| `pool_timeout`     | No       | `10`                   | 等待连接池中连接超时时间， `0` 代表无限制                                                                  |
...
PostgreSQL 连接器将 Prisma [数据模型] (../components/prisma-schema/data-model) 的 [标量类型（scalar types）](../components/prisma-schema/data-model#scalar-fields) 映射为数据库原生类型：
