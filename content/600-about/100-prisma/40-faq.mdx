...
是的。Prisma Client 基于 Prisma 的[查询引擎](。./concepts/components/prisma-engines/query-engine) ，它可以连接到任何提供正确 `_connector` 实现的数据源。将会有一些内置连接器，比如当前用于[PostgreSQL](的连接器。./concepts/database-connectors/postgresql)、[MySQL](。./concepts/database-connectors/mysql)和[ SQLite ](。./concepts/database-connectors/sqlite).
