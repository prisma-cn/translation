...
是的。 Prisma 客户端基于 Prisma 的[查询引擎](../concepts/components/prisma-engines/query-engine) ，可以提供*连接器实现*连接到任何数据源。将有内置连接器，如 [PostgreSQL](../concepts/database-connectors/postgresql), [MySQL](../concepts/database-connectors/mysql) 和 [SQLite](../concepts/database-connectors/sqlite)。
...
Prisma 客户端对如何准确的迁移数据库模式（例如，创建新表、更改列等）并不固执己见。你可以保留现有的系统迁移，并在每次模式迁移以后对数据库执行重复[自查](../concepts/components/introspection)。你还可以 Prisma Migrate 基于 Prisma 的声明性 [数据模型定义](../concepts/components/prisma-schema/data-model)。
