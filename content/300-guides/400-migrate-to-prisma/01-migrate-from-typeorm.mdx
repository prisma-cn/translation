...
如上所述，本指南是 TypeORM Express 示例的扩展变体，并使用相同的文件结构。 路由处理程序位于 `src/controller` 目录中。 它们被引入 `src/routes.ts` 核心文件中，该文件用于在 `src/index.ts` 中配置路由：
...
通过内省生成的模型精确的映射到了数据库表。在本节中，您将了解如何调整 Prisma 模型的命名并满足 [Prisma 的命名约定](../../reference/api-reference/prisma-schema-reference#naming-conventions)。
...
注意，每个返回的 `Post` 对象都包含与其关联的 `author` 的关系。 使用 TypeORM，，包含关系不是类型安全的。 例如，如果检索到的关系中存在拼写错误，您的数据库查询只会在运行阶段失败，TypeScript 编译器在此处不提供任何安全性。
