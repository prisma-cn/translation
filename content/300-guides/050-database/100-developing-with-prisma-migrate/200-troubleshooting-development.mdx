...
当文件系统中的**migrations 文件夹**和`_prisma_migrations`数据库中的表之间存在差异时，就会发生迁移历史冲突。
...
> **注意**: 检测 Schema 偏移(drift)需要[影子数据库](../../../concepts/components/prisma-migrate/shadow-database)，因此只能在开发环境中进行。

#### 修复开发环境的 Schema 偏移(drift)

如果对数据库做的修改不想保留，可以复制一份副本：
...
2. 复制 Prisma schema 中的变化，并生成一个新的迁移:
...
2. 生成一个新的迁移，以包括迁移历史中的自省(introspect)变化:
