---
title: 用Prisma Migrate进行团队开发
metaDescription: 当团队在一个项目上合作时，如何使用Prisma Migrate。
---
...
1. 拉取变更的 Prisma schema 和 `./prisma/migrations` 文件夹
...
下面的示例场景展示了三名开发人员的团队如何分享和合并对 Prisma schema 和迁移历史的更改。
...
1.  用你的改动创建一个新的迁移，提示你给它命名（`pacman-field`），并将新的迁移应用到你的本地数据库：
...
你的 "schema.prisma "和本地数据库现在包括你的团队的变化，迁移历史包括你的迁移：
...
仅仅从源头上控制`schema.prisma`文件是不够的，必须包括你的迁移历史。这是因为：

- 当你开始[自定义迁移](customizing-migrations)时，你的迁移历史包含了**不能在 Prisma schema 中表示的信息**。例如，你可以定制一个迁移，以减轻由破坏性变化引起的数据损失。
- `prisma migrate deploy`命令用于将更改部署到暂存、测试和生产环境，\_只运行迁移文件。Prisma Migrate 只使用 schema 文件来读取`url'和`provider'字段，而不是模型和字段。
