...
Baselining 是对数据库进行迁移历史初始化的过程:

- ✔ 在开始使用 Prisma Migrate 之前就已经存在
- ✔ 包含必须维护的数据（如生产环境），这意味着数据库不能被重置

Baselining 通知 Prisma Migrate 一个或多个迁移**已经被执行**. 避免了迁移执行时试图重复创建已经存在的表和字段时的报错。

> **注意**: 假定重置和 seed 开发环境的数据库是被允许的。

Baselining 是项目中 [对现有数据库添加 Prisma Migrate ](add-prisma-migrate-to-a-project) 的一部分。
...
Baselining 解决了这个问题， 它告诉 Prisma Migrate 假定这些迁移 **已经被执行**.
...
1. 跳过所有标记为 "应用 "的迁移，包括 baseline 迁移。
1. 应用所有 baseline 迁移 _后面_ 的新迁移。
