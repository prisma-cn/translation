---
title: '级联删除 (SQLite)'
metaTitle: '使用Prisma 和 SQLite，配置级联删除'
metaDescription: '通过遵循本实用指南中的逐步说明，了解如何使用Prisma和SQLite配置级联删除。'
---
...
本页将解释如何在 SQLite 数据库中配置[外键约束关系](https://www.sqlite.org/foreignkeys.html#fk_actions) (relations)的级联删除。

级联删除可以按照关系来配置删除行为(规则如， 一个用户被删除，则该用户的所有帖子会一同被删除)。设置后，数据库会严格遵守此级联删除规则。

在 SQLite 中，通常有五个选项用于配置删除行为(引用自 [SQLite docs](https://www.sqlite.org/foreignkeys.html#fk_actions)):
...
- 一个运行的[`sqlite3`](https://www.sqlite.org/cli.html)数据库服务器
...
接下来，确保 SQLite 数据库服务器正在运行。然后在终端执行以下命令，创建一个名为 `CascadingDeletesDemo`的新数据库：

```
sqlite3 CascadingDeletesDemo.db '.databases'
```

可以通过运行命令来验证数据库是否被创建，该命令列出了数据库中的所有表(现在没有表)：
