---
title: '外键(SQLite)'
metaTitle: '使用 Prisma 和 SQLite 配置外键'
metaDescription: '如何使用 Prisma 和 SQLite 一步一步配置外键。'
---

## 概览

介绍如何在 PostgreSQL 数据库中配置 [外键约束](https://www.sqlite.org/foreignkeys.html)。 外键表示数据库中的关联关系。
...
- SQLite 的 [`sqlite3`](https://www.sqlite.org/cli.html) 命令行客户端
...
在终端执行命令以创建一个名为 `ForeignKeyDemo` 的新数据库：
